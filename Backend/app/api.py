...
def select_examples(model: Any, features_df: pd.DataFrame, target: pd.Series) -> list[dict[str, Any]]:
    probabilities = pd.DataFrame(model.predict_proba(features_df), columns=model.classes_)
...
def create_app() -> Flask:
        ...
        return jsonify(
            {
                ...
                "input_format": {"features": {feature: "number" for feature in features}},
            }
        )
    ...
    @app.get("/examples")
    def examples():
        return jsonify({"examples": select_examples(model, bundle.features, bundle.target)})
    ...
    def predict():
        ...
        row, error = validate_prediction_values(values, features)
        if error:
            body, status = error
        ...
        return jsonify(build_prediction_response(model, class_labels, row, features, threshold, stats))
